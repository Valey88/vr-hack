  ...
  devtools((_, set) => ({ 
    ...
    registrationsOrder: async (data) => { 
      try { 
        ...
        toast.success("Успешно отправлено: " + response.data); // Уведомление об успешной отправке 
      } catch (error) { 
        console.log("Error:", error); 
        toast.error("Ошибка: " + (error.response?.data || error.message)); // Уведомление об ошибке 
      } 
    }, 
    uploads: async (data) => { 
